...
describe('+ AppController', () => {
  ...
  describe('+ GET: /status', () => {
    it('+ Services are online', function testStatus(done) {
      request.get('/status')
        .expect(200)
        .end((err, res) => {
          if (err) {
  ...
  describe('+ GET: /stats', () => {
    it('+ Correct statistics about db collections', function testStats(done) {
      request.get('/stats')
        .expect(200)
        .end((err, res) => {
          if (err) {
    ...
    it('+ Correct statistics about db collections [alt]', function testStatsAlt(done) {
      this.timeout(10000);
      Promise.all([dbClient.usersCollection(), dbClient.filesCollection()])
        .then(([usersCollection, filesCollection]) => {
          Promise.all([
